...
This mode allows for direct control of pixels to show information by colours, this includes the ability to control the brightness using various methods (Static, pulsing or blinking). The amount of pixels available for per-pixel control is defined by STRIP_NOTIFICATION_SIZE and due to memory constraints is limited. This can be equal to or smaller than the total strip size (STRIP_SIZE).

Methods include:
- Static (ON, OFF)
- Blink (Toggles ON & OFF)
- Pulser (Fades between ON & OFF)

Optional Features:
- "timeon": Auto time to turn pixel off
- "timeout": Timed that forces *all* notifications pixels off

The MQTT topics available for this mode are:

| Topic      | Type  | Rate  | Notes  |
| -------------    | ------------- | ----- | ----- |
| ./set/pixels/notifications | Commands | NA | Set parameters |
| ./status/pixels/notifications | Status | IfChanged, taskerTelePeriod | Receive parameters |

An example command to control the pixels is:
```cpp
{"pixelnum":2,"hue":120,"brt":100,"pulser":2000,"time_ms":6000}
```
where pixel 2 will be set to green on 100% brightness, pulsing for 6 seconds (i.e., On and Off three times).

There is no minimal command required to operate this method, however, without a pixelnum value the default command will assume *ALL* pixels should be updated.

Full list of JSON Formatted Commands

| Command Key      | Accepts Value | Optional  | Notes  |
| -------------    | ------------- | :-----:| ----- |
| pixelnum         | 0..MAX, "all", [array] | N* | [1,2,..., MAX], MAX is STRIP_NOTIFICATION_SIZE, N* default is "all" |
| hue              | 0..360, [array] | y |  |
| sat              | 0..100, [array] | y |  |
| brt              | 0..100, [array] | y |  |
| rgb              | "RRGGBB", "#RRGGBB", [array] | y |  ["RRGGBB","RRGGBB"]  |
| timeout_enable   | 0/1, "false"/"true", "OFF"/"ON" | y | Resets all pixels, overrides "timeon" |
| timeout_secs     | 0..65535 | y | Resets time for all pixels, overrides "timeon" |
| pusler           | 0..255 | y | Default unit is secs |
| pusler_ms        | 0..65535 | y |  |
| pusler_secs      | 0..255 | y |  |
| blink            | 0..255 | y | Default unit is secs |
| blink_ms         | 0..65535 | y |  |
| blink_secs       | 0..255 | y |  |
| timeon           | 0..255 | y | Default unit is secs |
| timeon_ms        | 0..65535 | y |  |
| timeon_secs      | 0..255 | y |  |
...
Idea?? Use topic as key and value without json
{"pixelnum":"0"}
would be
set/pixels/notifications/pixelnum -m 0

Where commands are not optional, assume/use "all"
